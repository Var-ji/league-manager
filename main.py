...
def get_sizes():
	while 1:
		try:
			group_number = int(raw_input("Enter number of groups: "))
		except ValueError:
	...
	while 1:
		try:
			group_size = int(raw_input("Enter size of each group: "))
		except ValueError:
			print("Enter an integer.")
			continue
		if group_size < 3:
...
def make_groups(members_tuple, group_number, group_size):
	...
	while members_list:
		...
		members_list = [x for x in members_list if x not in group]
